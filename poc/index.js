...
class SolanaWalletReporter {
  ...
  analyzeTransaction(tx, walletAddress) {
    ...
    try {
        ...
        // Token Program
          ...
          if (ix.parsed?.type === 'transfer' || ix.parsed?.type === 'transferChecked') {
            ...
            result.tokenTransfers.push({
              amount: info.amount || info.tokenAmount?.uiAmount,
              mint: info.mint || 'Unknown Token'
            });
      ...
      // Parse inner instructions for more details
      if (tx.meta?.innerInstructions) {
        for (const inner of tx.meta.innerInstructions) {
          for (const ix of inner.instructions) {
            if (ix.parsed?.type === 'transfer' && ix.program === 'spl-token') {
              if (!result.type.includes('Token') && !result.type.includes('Swap')) {
  ...
  generateMarkdownReport(allData) {
  ...
  async saveReport(markdownContent) {
    ...
    if (this.daysToFetch === 0) {
      ...
      const weekNumber = this.getWeekNumber(new Date());
      const year = new Date().getFullYear();
      filename = `solana-wallet-${year}-W${weekNumber}.md`;
    }
  ...
  async run() {
    try {
      console.log('Starting Solana Wallet Reporter...');
      ...
      const markdown = this.generateMarkdownReport(allData);
